...
<head>
    ...
    <style>
        * { scrollbar-width: thin; scrollbar-color: #e5e7eb transparent; }
        ...
        .card-hover.selected { transform: translateY(-3px); box-shadow: 0 0 0 2px #c94f2e, 0 12px 25px -8px rgba(201,79,46,0.2); }
        .time-btn { transition: all 0.2s ease; }
        .time-btn:hover:not(:disabled) { transform: translateY(-2px); box-shadow: 0 4px 12px rgba(201,79,46,0.2); }
        .time-btn.selected { transform: translateY(-2px); box-shadow: 0 0 0 2px #c94f2e, 0 4px 12px rgba(201,79,46,0.15); }
...
<body class="bg-gradient-to-br from-stone-50 via-orange-50 to-stone-100 min-h-screen font-sans text-stone-800">
    ...
    <div id="successOverlay" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
        <div class="bg-white rounded-3xl shadow-2xl max-w-md w-full p-8 text-center animate-bounce-in">
            ...
            <h2 class="text-2xl font-bold text-stone-800 mb-2">Agendamento Confirmado!</h2>
            <p class="text-stone-500 mb-6">{{ session('success') }}</p>
    ...
    <div class="min-h-screen flex flex-col">
        <header class="pt-8 pb-4 px-4 text-center">
            ...
            <h1 class="text-2xl font-bold text-stone-800">Agende seu Horário</h1>
            <p class="text-stone-500 text-sm mt-1">Escolha o profissional, serviço e melhor horário para você</p>
        </header>
        ...
        <main class="flex-1 px-4 pb-12">
            <div class="max-w-xl mx-auto">
                <form id="bookingForm" method="POST" action="{{ route('public.booking.store') }}">
                    ...
                    <div class="bg-white/80 backdrop-blur-md rounded-3xl shadow-xl border border-white/60 p-6 sm:p-8">
                        ...
                        <div class="flex items-center justify-between mb-8">
                            <div class="flex items-center gap-1">
                                ...
                                <div class="step-dot" data-step="2"><div class="w-8 h-8 rounded-full bg-stone-200 text-stone-500 flex items-center justify-center text-xs font-bold">2</div></div>
                                <div class="step-line" data-step="2"><div class="h-0.5 w-8 bg-stone-200"></div></div>
                                <div class="step-dot" data-step="3"><div class="w-8 h-8 rounded-full bg-stone-200 text-stone-500 flex items-center justify-center text-xs font-bold">3</div></div>
                                <div class="step-line" data-step="3"><div class="h-0.5 w-8 bg-stone-200"></div></div>
                                <div class="step-dot" data-step="4"><div class="w-8 h-8 rounded-full bg-stone-200 text-stone-500 flex items-center justify-center text-xs font-bold">4</div></div>
                            </div>
                        ...
                        <div id="step1" class="step-content step-enter">
                            <h2 class="text-lg font-semibold text-stone-800 mb-1">Identifique-se</h2>
                            <p class="text-sm text-stone-400 mb-5">Informe seu nome e CPF para continuar</p>

                            <div class="space-y-4">
                                <div>
                                    <label class="block text-sm font-medium text-stone-700 mb-1">Nome Completo</label>
                                    <input type="text" id="step1Name" class="w-full rounded-xl border-2 border-stone-200 bg-white/80 shadow-sm focus:border-brand-400 focus:ring focus:ring-brand-200/30 p-3 text-sm transition-all" placeholder="Digite seu nome">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-stone-700 mb-1">CPF</label>
                                    <input type="text" id="step1Cpf" class="w-full rounded-xl border-2 border-stone-200 bg-white/80 shadow-sm focus:border-brand-400 focus:ring focus:ring-brand-200/30 p-3 text-sm transition-all" placeholder="000.000.000-00">
                                </div>
                                <div id="step1PhoneGroup">
                                    <label class="block text-sm font-medium text-stone-700 mb-1">Telefone <span class="text-xs text-stone-400">(opcional)</span></label>
                                    <input type="text" id="step1Phone" class="w-full rounded-xl border-2 border-stone-200 bg-white/80 shadow-sm focus:border-brand-400 focus:ring focus:ring-brand-200/30 p-3 text-sm transition-all" placeholder="(11) 99999-8888">
                                </div>
                            </div>
                            ...
                            <div id="step1Welcome" class="hidden mt-6 p-6 bg-gradient-to-r from-emerald-50 to-green-50 rounded-2xl border border-emerald-200 text-center">
                                <div class="w-16 h-16 rounded-full bg-emerald-200 flex items-center justify-center mx-auto mb-3">
                                    <svg class="w-8 h-8 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5.121 17.804A13.937 13.937 0 0112 16c2.5 0 4.847.655 6.879 1.804M15 10a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                                </div>
                                <h3 class="text-xl font-bold text-emerald-800 mb-1">Bem-vinda, <span id="welcomeName"></span>!</h3>
                                <p class="text-sm text-emerald-600" id="welcomeCpf"></p>
                            </div>
                            ...
                            <button type="button" onclick="nextStep(2)" id="step1Next" class="mt-3 w-full bg-stone-200 text-stone-400 font-semibold rounded-xl py-3 cursor-not-allowed transition-all duration-200 text-sm hidden">
                                Continuar
                            </button>
                        </div>
                        ...
                        <div id="step2" class="step-content hidden">
                            <h2 class="text-lg font-semibold text-stone-800 mb-1">Escolha a Atendente</h2>
                            <p class="text-sm text-stone-400 mb-5">Quem você gostaria de atender você?</p>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3" id="professionalsGrid">
                                @foreach($users as $u)
                                <div class="professional-card card-hover relative bg-white rounded-xl border-2 border-stone-100 p-4 cursor-pointer" data-value="{{ $u->id }}" onclick="selectProfessional(this)">
                                    <div class="flex items-center gap-3">
                                        <div class="w-12 h-12 rounded-full bg-gradient-to-br from-brand-100 to-brand-200 flex items-center justify-center text-brand-600 font-bold text-lg shrink-0">
                                            {{ strtoupper(substr($u->name, 0, 2)) }}
                                        </div>
                                        <div>
                                            <div class="font-semibold text-stone-800 text-sm">{{ $u->name }}</div>
                                            <div class="text-xs text-stone-400">Atendente</div>
                                        </div>
                                    </div>
                            ...
                            <div class="flex gap-3 mt-6">
                                <button type="button" onclick="prevStep(1)" class="flex-1 bg-stone-100 text-stone-600 font-medium rounded-xl py-3 hover:bg-stone-200 transition-all duration-200 text-sm">Voltar</button>
                                <button type="button" onclick="nextStep(3)" disabled id="step2Next" class="flex-1 bg-stone-200 text-stone-400 font-semibold rounded-xl py-3 cursor-not-allowed transition-all duration-200 text-sm">
                        ...
                        <div id="step3" class="step-content hidden">
                            <h2 class="text-lg font-semibold text-stone-800 mb-1">Escolha os Serviços</h2>
                            <p class="text-sm text-stone-400 mb-5">Selecione um ou mais procedimentos</p>
                            <div class="grid grid-cols-1 gap-3" id="servicesGrid">
                                @foreach($services as $s)
                                <div class="service-card card-hover relative bg-white rounded-xl border-2 border-stone-100 p-4 cursor-pointer" data-value="{{ $s->id }}" onclick="toggleService(this)">
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div class="font-semibold text-stone-800 text-sm">{{ $s->name }}</div>
                                                <div class="flex items-center gap-2 mt-0.5">
                                                    <span class="text-xs text-stone-400">{{ $s->duration_min }} min</span>
                                                    <span class="text-xs text-stone-300">•</span>
                                                    <span class="text-xs font-medium text-brand-600">R$ {{ number_format($s->price, 2, ',', '.') }}</span>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <p id="selectedServicesCount" class="text-xs text-stone-400 mt-2">Nenhum serviço selecionado</p>
                            <div id="serviceIdsContainer"></div>
                            <div class="flex gap-3 mt-6">
                                <button type="button" onclick="prevStep(2)" class="flex-1 bg-stone-100 text-stone-600 font-medium rounded-xl py-3 hover:bg-stone-200 transition-all duration-200 text-sm">Voltar</button>
                                <button type="button" onclick="nextStep(4)" disabled id="step3Next" class="flex-1 bg-stone-200 text-stone-400 font-semibold rounded-xl py-3 cursor-not-allowed transition-all duration-200 text-sm">
                        ...
                        <div id="step4" class="step-content hidden">
                            <h2 class="text-lg font-semibold text-stone-800 mb-1">Escolha a Data e Horário</h2>
                            <p class="text-sm text-stone-400 mb-5">Selecione o melhor dia e horário disponível</p>
                            ...
                            <div id="timeSlotsContainer">
                                <label class="block text-sm font-medium text-stone-700 mb-2">Horários Disponíveis</label>
                                ...
                                <div id="timeSlotsGrid" class="time-grid gap-2">
                                    <div class="col-span-full text-center py-8 text-stone-400 text-sm">
                                        <svg class="w-10 h-10 mx-auto mb-2 text-stone-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                                        Selecione uma data para ver os horários
                                    </div>
                                </div>
                            </div>

                            <div class="mt-6 p-4 bg-stone-50 rounded-xl border border-stone-100" id="summaryBox">
                                <h3 class="text-xs font-semibold text-stone-400 uppercase tracking-wider mb-3">Resumo do Agendamento</h3>
                                <div class="space-y-2 text-sm">
                                    <div class="flex items-center gap-2"><span class="text-stone-400 w-20 shrink-0">Cliente:</span><span class="font-medium text-stone-800" id="resumoCliente">—</span></div>
                                    <div class="flex items-center gap-2"><span class="text-stone-400 w-20 shrink-0">Atendente:</span><span class="font-medium text-stone-800" id="resumoAtendente">—</span></div>
                                    <div class="flex items-center gap-2"><span class="text-stone-400 w-20 shrink-0">Serviço:</span><span class="font-medium text-stone-800" id="resumoServico">—</span></div>
                                    <div class="flex items-center gap-2"><span class="text-stone-400 w-20 shrink-0">Data/Hora:</span><span class="font-medium text-stone-800" id="resumoData">—</span></div>
                                </div>
                            </div>

                            <div class="flex gap-3 mt-6">
                                <button type="button" onclick="prevStep(3)" class="flex-1 bg-stone-100 text-stone-600 font-medium rounded-xl py-3 hover:bg-stone-200 transition-all duration-200 text-sm">Voltar</button>
                                <button type="submit" disabled id="step4Submit" class="flex-1 bg-stone-200 text-stone-400 font-semibold rounded-xl py-3 cursor-not-allowed transition-all duration-200 text-sm">
        ...
        <footer class="pb-8 px-4 text-center">
            <div class="max-w-xl mx-auto flex items-center justify-center gap-6 text-xs text-stone-400">
                <span>© {{ date('Y') }} Nubi Estética</span>
    ...
    <script>
    var stepLabels = ['Cliente', 'Atendente', 'Serviço', 'Data e Horário'];
    var currentStep = 1;
    ...
    function updateSteps(step) {
        ...
        document.querySelectorAll('.step-dot').forEach(function(dot, i) {
            ...
            if (num < step) {
                ...
                circle.className = 'w-8 h-8 rounded-full bg-stone-200 text-stone-500 flex items-center justify-center text-xs font-bold';
                circle.textContent = num;
                if (line) line.className = 'h-0.5 w-8 bg-stone-200';
            }
        });
    ...
    function toggleService(el) {
        ...
        if (count > 0) {
            ...
            btn.disabled = true;
            btn.className = 'flex-1 bg-stone-200 text-stone-400 font-semibold rounded-xl py-3 cursor-not-allowed transition-all duration-200 text-sm';
        ...
        fetch('/agendar/slots?user_id=' + userId + '&service_id=' + serviceId + '&date=' + date)
            ...
            .then(function(data) {
                ...
                if (data.slots.length === 0) {
                    grid.innerHTML = '<div class="col-span-full text-center py-8 text-stone-400 text-sm"><svg class="w-10 h-10 mx-auto mb-2 text-stone-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>Nenhum horário disponível nesta data</div>';
                } else {
                    data.slots.forEach(function(slot) {
                        var b = document.createElement('button');
                        b.type = 'button';
                        b.className = 'time-btn bg-white border-2 border-stone-200 rounded-xl py-2.5 text-sm font-medium text-stone-700 hover:border-brand-300';
        ...
        for (var i = 0; i < 7; i++) {
            ...
            chip.onclick = function() {
                document.querySelectorAll('#quickDates button').forEach(function(b) {
                    b.className = 'px-3 py-1.5 rounded-lg text-xs font-medium border-2 border-stone-200 text-stone-600 hover:border-brand-300 hover:text-brand-600 hover:bg-brand-50 transition-all';
                });
                this.className = 'px-3 py-1.5 rounded-lg text-xs font-medium border-2 border-brand-400 text-brand-700 bg-brand-50 transition-all';
