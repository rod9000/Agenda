...
@section('header')
    <div class="flex justify-between items-center">
        <h2 class="font-semibold text-xl text-brand-800 dark:text-brand-200 leading-tight">Evolution API — WhatsApp</h2>
    </div>
@endsection
...
<div class="py-6">
    <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="card-pastel">
            <h3 class="font-semibold text-brand-700 dark:text-brand-300 mb-4">Configuração do Servidor</h3>
            ...
            <form method="POST" action="{{ route('admin.settings.evolution.store') }}">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="evolution_instance_name" value="{{ old('evolution_instance_name', $company->evolution_instance_name) }}" placeholder="agenda-estetica" required class="input-pastel">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-stone-400 dark:text-stone-500 mt-1">Selecione o tipo de WhatsApp que será conectado</p>
                        @error('whatsapp_type') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                    </div>
                    <div>
                        <label class="label">URL do Webhook</label>
                        <input type="url" name="evolution_webhook_url" value="{{ old('evolution_webhook_url', $company->evolution_webhook_url) }}" placeholder="{{ url('/api/webhook/evolution') }}" class="input-pastel">
                        @error('evolution_webhook_url') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                        <p class="text-xs text-stone-400 dark:text-stone-500 mt-1">URL que a Evolution API usará para enviar as mensagens recebidas.</p>
                    </div>
                </div>
        ...
        {{-- Connection Status --}}
        <div class="card-pastel">
            <h3 class="font-semibold text-brand-700 dark:text-brand-300 mb-4">Conexão WhatsApp</h3>

            <div id="connectionStatus" class="flex items-center gap-3 mb-4">
                @if($status['connected'] ?? false)
                    <span class="w-3 h-3 rounded-full bg-emerald-500 inline-block"></span>
                    <span class="text-emerald-600 dark:text-emerald-400 font-medium">Conectado</span>
            ...
            @if(!$company->evolutionConfigured())
                <p class="text-sm text-stone-500 dark:text-stone-400">Salve as configurações do servidor para conectar.</p>
            ...
            @else
                <div class="space-y-4">
                    ...
                    <div id="qrcodeContainer" class="hidden flex flex-col items-center gap-4 p-6">
                        <p class="text-sm text-stone-500 dark:text-stone-400">Escaneie o QR Code abaixo com seu WhatsApp:</p>
                        <div id="qrcodeImage" class="bg-white dark:bg-stone-700 p-4 rounded-2xl shadow-sm"></div>
                        <p class="text-xs text-stone-400 dark:text-stone-500">Abra o WhatsApp > Menu > Aparelhos conectados > Conectar um dispositivo</p>
                        ...
                        <div id="qrcodeLoading" class="hidden text-sm text-brand-600 dark:text-brand-400">Aguardando conexão...</div>
                    </div>
                </div>
            @endif
        </div>
        ...
        {{-- Webhook --}}
        <div class="card-pastel">
            <h3 class="font-semibold text-brand-700 dark:text-brand-300 mb-4">Webhook</h3>
            <p class="text-sm text-stone-500 dark:text-stone-400 mb-4">
                Registre a URL de webhook no servidor Evolution para receber mensagens do WhatsApp.
            </p>

            @if($company->evolution_webhook_url)
            <div class="mb-4 text-sm space-y-1">
                <p class="text-stone-700 dark:text-stone-300"><strong>URL registrada:</strong> <code class="bg-stone-100 dark:bg-stone-700 px-2 py-0.5 rounded">{{ $company->evolution_webhook_url }}</code></p>
            </div>
            @endif
            ...
            <div class="flex gap-3">
                ...
                @else
                <p class="text-sm text-stone-500 dark:text-stone-400">Salve as configurações do servidor primeiro.</p>
                @endif
            </div>
...
function connectInstance() {
    ...
    .then(function(data) {
        ...
        if (data.qrcode) {
            ...
            qrImage.innerHTML = '<div class="text-center"><p class="text-lg font-bold text-brand-700 dark:text-brand-300">' + data.pairingCode + '</p><p class="text-xs text-stone-500 mt-1">Código de pareamento</p></div>';
        }
...
function setWebhook() {
    ...
    .then(function(data) {
        ...
        if (data.success) {
            result.className = 'mt-3 text-sm text-emerald-600 dark:text-emerald-400';
