...
@section('header')
    <div class="flex justify-between items-center">
        <h2 class="font-semibold text-xl text-brand-800 dark:text-brand-200 leading-tight">Mensagens do Bot</h2>
...
<div class="py-6">
    <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="card-pastel mb-4">
            <form method="GET" action="{{ route('admin.bot-messages.index') }}" class="flex gap-3 flex-wrap">
                <input type="text" name="search" value="{{ $search }}" placeholder="Buscar por telefone ou nome..." class="input-pastel flex-1 min-w-[200px]">
        ...
        <div class="card-pastel p-0 overflow-hidden">
            <div class="p-4 border-b border-brand-100 dark:border-stone-700 bg-brand-50/30 dark:bg-stone-800">
                <h3 class="font-semibold text-brand-700 dark:text-brand-300">Conversas</h3>
            </div>
            ...
            @if($conversations->isEmpty())
                <div class="p-8 text-center">
                    ...
                    <p class="text-brand-400 dark:text-brand-500 text-sm">Nenhuma conversa encontrada.</p>
                </div>
            @else
                <div class="divide-y divide-brand-100/50 dark:divide-stone-700">
                    @foreach($conversations as $conversation)
                        <a href="{{ route('admin.bot-messages.show', $conversation) }}" class="block p-4 hover:bg-brand-50/50 dark:hover:bg-stone-700/50 transition">
                            <div class="flex justify-between items-start">
                                ...
                                <div class="text-right ml-4 shrink-0">
                                    ...
                                    <div class="mt-1">
                                        <span class="px-2 py-0.5 text-xs rounded-full {{ $conversation->state === 'initial' ? 'bg-stone-200 text-stone-600 dark:bg-stone-700 dark:text-stone-400' : 'bg-brand-100 text-brand-700 dark:bg-brand-900/30 dark:text-brand-400' }}">
                                            {{ $conversation->state }}
                                        </span>
                                    </div>
                                </div>
...
{{-- Modal Nova Conversa --}}
<div id="newConversationModal" class="hidden fixed inset-0 z-50 overflow-y-auto" aria-modal="true">
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        <div class="fixed inset-0 bg-stone-900/50 dark:bg-stone-950/75 transition-opacity" onclick="document.getElementById('newConversationModal').classList.add('hidden')"></div>
        <div class="inline-block align-bottom bg-white dark:bg-stone-800 rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
            <div class="p-6">
                <h3 class="text-lg font-semibold text-brand-700 dark:text-brand-300 mb-4">Iniciar Nova Conversa</h3>
