...
<div class="py-6">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div class="card-pastel text-center">
                <div class="text-3xl font-bold text-sky-600">{{ $customer->anamnesisForms->count() }}</div>
                <div class="text-sm text-stone-500 mt-1">Fichas de Anamnese</div>
            </div>
            <div class="card-pastel text-center">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="md:col-span-2 space-y-6">
                ...
                @if($anamnesisForms->isNotEmpty())
                <div class="card-pastel p-0 overflow-hidden">
                    <div class="p-4 border-b border-brand-100 bg-brand-50/30">
                        <h3 class="font-semibold text-brand-700">Fichas de Anamnese</h3>
                    </div>
                    <div class="divide-y divide-brand-100">
                        @foreach($anamnesisForms as $form)
                        <div class="p-4 flex justify-between items-center">
                            <div>
                                <span class="text-sm font-medium">{{ $form->created_at->format('d/m/Y H:i') }}</span>
                                <span class="text-sm text-stone-500 ml-2">por {{ $form->answeredBy?->name ?? '—' }}</span>
                            </div>
                            <div class="space-x-2">
                                <a href="{{ route('admin.anamnesis.show', $form) }}" class="text-sm text-brand-600 hover:text-brand-800">Visualizar</a>
                            </div>
                        </div>
                        @endforeach
                    </div>
